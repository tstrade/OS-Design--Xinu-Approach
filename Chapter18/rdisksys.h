...
#define RD_PRIO         200        /* Priority for comm. process    */
...
/* Definition of a buffer with a header that allows the same node   */
/*   to be used as a request on the request queue, an item in the   */
/*   cache, or a node on the free list of buffers                   */
struct rdbuff {
    struct rdbuff *rd_next;         /* Ptr to next node on a list   */
    struct rdbuff *rd_prev;         /* Ptr to prev node on list     */
    ...
    int32  rd_status;               /* Is buffer current valid?     */
...
struct rdscblk {
    int32  rd_state;                /* State of the device          */
    char   rd_id[RD_IDLEN];         /* Disk ID current being used   */
    ...
    struct rdbuff *rd_rtprev;       /*    and previous              */

    /* Cache head and tail*/
    struct rdbuff *rd_chnext;       /* Head of cache list: next     */
    struct rdbuff *rd_chprev;       /*    and previous              */
    struct rdbuff *rd_ctnext;       /* Tail of cache list: next     */
    struct rdbuff *rd_ctprev;       /*    and previous              */

    /* Free list head (singly-linked)*/
    struct rdbuff *rd_free;         /* Pointer to free list         */

    pid32  rd_comproc;              /* PID of the comm. process     */
    bool8  rd_comruns;              /* Has comm. process started?   */
    sid32  rd_availsem;             /* Semaphore for avail. buffers */
    sid32  rd_reqsem;               /* Semaphore for requests       */
...
/* Control functions for a remote file psuedo device */
#define RDS_CTL_DEL     1           /* Delete (erase) an entire disk*/
#define RDS_CTL_SYNC    2           /* Write all pending blocks     */

/********************************************************************/
/*    Definition of messages exchanges with the remote disk server  */ 
/********************************************************************/
